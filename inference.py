...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        # Iterating over every possible legal posistion of the ghosts
        for position in self.legalPositions:
            # Calculating the manhattan distance between possible ghost position and pacman's position
            trueDistance = util.manhattanDistance(position, pacmanPosition)
            # Getting the probability of the Noisy distance this truedistance
            if emissionModel[trueDistance] > 0:
                """Updating new beleif of this position by Multiplying the probability of this truedistance with the 
                    previous beleif of this ghost position
                    """
                allPossible[position] = emissionModel[trueDistance] * self.beliefs[position]

        # Checking if ghost is captured by pacman with noisy distance is None
        if noisyDistance == None:
            # Setting beleif of jail position with max Probability i.e 1
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        "*** YOUR CODE HERE ***"
        allPossible = util.Counter()
        # Iterate over all the possible legal ghost positions
        for oldPos in self.legalPositions:
            # Getting all the probabililties for each new positions from the current game state and ghost position
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, oldPos))
            # Iterating over all the new positions
            for newPos,prob in newPosDist.items():
                # Updating beleif of the new position by adding product of its probability and beleif of the old position
